...
def move_commas_to_start(ddl: str) -> str:
    ...
    for line in lines:
        ...
        # Detect start of column section
        if stripped_line.endswith('('):
            inside_columns = True
            transformed_lines.append(line)
            continue

        # Detect end of column section
        ...
        if inside_columns:
            ...
            if first_column:
                ...
                # Subsequent columns: add leading comma
...
def launch_gui():
    ...
    def convert_file():
        ...
        if input_file:
            output_file = filedialog.asksaveasfilename(defaultextension="_formatted.sql", filetypes=[("SQL Files", "*.sql")])
            if output_file:
                process_file(input_file, output_file)
                messagebox.showinfo("Success", f"File processed: {output_file}")
    ...
    tk.Label(frame, text="File/Folder Operations:").pack(pady=10)
    tk.Button(frame, text="Convert File", command=convert_file).pack(pady=5)
    tk.Button(frame, text="Convert Folder", command=convert_folder).pack(pady=5)
